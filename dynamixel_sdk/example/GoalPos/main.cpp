...
int main()
{
    ...
    int GoalPos = 4000; // 9-o-clock position for 54-series
    ...
    DXL.WriteByte(1, LL_TORQUE_ENABLE, 1, 0);
    usleep(CONTROL_PERIOD*100);
    // set initial Goal Position to 0
    DXL.WriteDWord(1, LL_GOAL_POSITION, 0, 0);
...
/*
    DXL.WriteDWord(1, LL_MAX_POSITION, 0, 0);
    usleep(CONTROL_PERIOD*100);
    DXL.WriteDWord(1, LL_MIN_POSITION, 0, 0);
    usleep(CONTROL_PERIOD*100);
*/

    DXL.WriteWord(1, LL_GOAL_CURRENT, 0, 0);
    usleep(CONTROL_PERIOD*100);
    ...
    while(1)
    {
        ...
        if(_getch() == 0x1b)
            break;
        DXL.WriteDWord(1, LL_GOAL_POSITION, 3000, 0);   //move to 9-o-clock position     
        //DXL.WriteDWord(1, GOAL_POSITION, GoalPos, 0);   //move to 9-o-clock position     
	printf( "moving...\n" );	
	usleep(CONTROL_PERIOD*500);

        usleep(1000000);
        DXL.WriteDWord(1, LL_GOAL_POSITION, 0, 0);   //move to 9-o-clock position     
        long int max_pos = 0;
        long int state = 0; 
        DXL.ReadDWord(1, LL_MAX_POSITION, &max_pos, 0);   //move to 9-o-clock position     
        while(1){
          int cur = 0; 
          DXL.ReadWord(1, LL_PRESENT_CURRENT, &cur, 0);   //move to 9-o-clock position     
	  usleep(CONTROL_PERIOD*500);
          DXL.ReadDWord(1, LL_PRESENT_POSITION, &state, 0);   //move to 9-o-clock position     
	  usleep(CONTROL_PERIOD*500);
          // printf("Current = %hd\n",cur);
          int thresh_cur = 12;
          int16_t ncur = (int16_t)cur;
//          if( ncur > 0 ) printf("+\n");
//          else printf("-\n");

          int kp = -50;
          int e = (int)ref - (int)state;
          ref = (int)state;
          


          printf("ref = %d\n",ref);
          DXL.WriteDWord(1, LL_GOAL_POSITION, ref, 0);   //move to 9-o-clock position     
	  usleep(CONTROL_PERIOD*500);
          usleep(10000);
        }
       printf("Max pos = %ld\n", max_pos);
